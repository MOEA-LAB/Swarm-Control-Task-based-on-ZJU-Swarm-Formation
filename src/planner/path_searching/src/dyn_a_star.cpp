...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt,
                                                  Vector3i &start_idx, Vector3i &end_idx)
{
                        ...
                        bool use_esdf_check)
{
  ...
  //     cout << "start_pt=" << start_pt.transpose() << " end_pt=" << end_pt.transpose()
  //     << endl;
  ...
  while (!openSet_.empty())
  {
        ...
        current->index(2) == endPtr->index(2))
    {
      ...
      //     ROS_WARN("Time consume in A star path finding is %f", (time_2 -
      //     time_1).toSec() );
      gridPath_ = retrievePath(current);
      return true;
    }
    current->state =
        GridNode::CLOSEDSET;  // move current node from open set to closed set.
    ...
    for (int dx = -1; dx <= 1; dx++)
      for (int dy = -1; dy <= 1; dy++)
        for (int dz = -1; dz <= 1; dz++)
        {
          ...
          if (use_esdf_check)
          {
            if (checkOccupancy_esdf(Index2Coord(neighborPtr->index))) continue;
          }
          else
          {
            if (checkOccupancy(Index2Coord(neighborPtr->index))) continue;
          }
...
vector<Vector3d> AStar::astarSearchAndGetSimplePath(const double step_size,
                                                    Vector3d start_pt, Vector3d end_pt)
{
  ...
  if ((path[0] - start_pt).norm() > 0.5)
  {
    ...
    {
      ROS_ERROR("Failed to find a path after 10 tries.");
      assert(false);
    };
    path = getPath();
  }
  ...
  if (is_show_debug)
  {
    ...
    for (int i = 0; i < n1; i++) cout << simple_path[i].transpose() << endl;
  }
  ...
  if (is_show_debug)
  {
    ...
    for (int i = 0; i < n2; i++) cout << simple_path[i].transpose() << endl;
  }
  ...
  if (is_show_debug)
  {
    ...
    for (int i = 0; i < n3; i++) cout << simple_path[i].transpose() << endl;
  }
