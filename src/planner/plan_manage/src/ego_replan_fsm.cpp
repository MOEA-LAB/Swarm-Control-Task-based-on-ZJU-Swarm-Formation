...
{
EGOReplanFSM::~EGOReplanFSM() { result_file_.close(); }
void EGOReplanFSM::init(ros::NodeHandle &nh)
{
  ...
  nh.param(
      "fsm/result_file", result_fn_,
      string("/home/zuzu/Documents/Benchmark/21-RSS-ego-swarm/2.24/ego/ego_swarm.txt"));
  ...
  swarm_relative_pts_s_[4][1] = -0.7291;
  ...
  // Similarly for swarm_relative_pts_y_
  swarm_relative_pts_y_[0][0] = -0.4815;
  swarm_relative_pts_y_[0][1] = 0.5139;
  swarm_relative_pts_y_[0][2] = 0.0;
  swarm_relative_pts_y_[1][0] = 0.5156;
  swarm_relative_pts_y_[1][1] = 0.5122;
  ...
  swarm_relative_pts_y_[5][1] = -1.2715;
  swarm_relative_pts_y_[5][2] = 0.0;
  swarm_relative_pts_y_[6][0] = 0.000;
  swarm_relative_pts_y_[6][1] = 0.300;
  swarm_relative_pts_y_[6][2] = 0.0;

  // Similarly for swarm_relative_pts_u_
  ...
  swarm_relative_pts_u_[5][1] = -1.2080;
...
void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
{
  ...
  switch (exec_state_)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      if ((local_target_pt_ - end_pt_).norm() <
          0.1)  // local target close to the global target
      {
        if (t_cur > info->duration - 0.2)
        {
          ...
          result_file_
              ...
              << (ros::Time::now() - planner_manager_->global_start_time_).toSec() << "\t"
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
{
  ...
  {
    // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is considered
    // valid and will get checked.
    if (t_cur < t_2_3 && t >= t_2_3) break;
    ...
    for (size_t id = 0; id < planner_manager_->traj_.swarm_traj.size(); id++)
    {
      ...
      if (t_X > planner_manager_->traj_.swarm_traj.at(id).duration) continue;
...
void EGOReplanFSM::RecvBroadcastPolyTrajCallback(const traj_utils::PolyTrajConstPtr &msg)
{
  ...
  {
    ROS_WARN("Time stamp diff: Local - Remote Agent %d = %fs", msg->drone_id,
