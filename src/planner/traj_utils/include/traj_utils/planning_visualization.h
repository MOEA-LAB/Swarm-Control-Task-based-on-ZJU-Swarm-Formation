...
class PlanningVisualization
{
 private:
  ...
  ros::Subscriber drone_0_odom_sub_, drone_1_odom_sub_, drone_2_odom_sub_,
      drone_3_odom_sub_;
  ros::Subscriber drone_4_odom_sub_, drone_5_odom_sub_, drone_6_odom_sub_,
      drone_7_odom_sub_;
  ...
  geometry_msgs::PoseStamped last_turn_msg;
 ...
 public:
  ...
  void displayGlobalPathList(vector<Eigen::Vector3d> global_pts, const double scale,
                             int id);
  ...
  void displayArrowList(ros::Publisher &pub, const vector<Eigen::Vector3d> &list,
                        double scale, Eigen::Vector4d color, int id);
  void displayInitPathListDebug(vector<Eigen::Vector3d> init_pts, const double scale,
                                int id);

  void displayIntermediatePt(std::string type, Eigen::MatrixXd &pts, int id,
                             Eigen::Vector4d color);
  void displayIntermediateGrad(std::string type, Eigen::MatrixXd &pts,
                               Eigen::MatrixXd &grad, int id, Eigen::Vector4d color);
  // void displayNewArrow(ros::Publisher& guide_vector_pub,
  // ego_planner::PolyTrajOptimizer::Ptr optimizer);
};
}  // namespace ego_planner
#endif