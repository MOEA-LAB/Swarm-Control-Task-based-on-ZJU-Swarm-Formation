...
PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh)
{
  ...
  intermediate_grad0_pub =
      nh.advertise<visualization_msgs::MarkerArray>("grad0_dur_opt", 10);
  intermediate_pt1_pub = nh.advertise<visualization_msgs::Marker>("pt1_dur_opt", 10);
  intermediate_grad1_pub =
      nh.advertise<visualization_msgs::MarkerArray>("grad1_dur_opt", 10);
  intermediate_grad_smoo_pub =
      nh.advertise<visualization_msgs::MarkerArray>("smoo_grad_dur_opt", 10);
  intermediate_grad_dist_pub =
      nh.advertise<visualization_msgs::MarkerArray>("dist_grad_dur_opt", 10);
  intermediate_grad_feas_pub =
      nh.advertise<visualization_msgs::MarkerArray>("feas_grad_dur_opt", 10);
  intermediate_grad_swarm_pub =
      nh.advertise<visualization_msgs::MarkerArray>("swarm_grad_dur_opt", 10);

  swarm_formation_visual_pub =
      nh.advertise<visualization_msgs::MarkerArray>("swarm_graph_visual", 10);

  t_init = ros::Time::now();

  last_turn_msg.pose.position.x = 0;
  last_turn_msg.pose.position.y = 0;
  ...
  for (int i = 0; i < formation_size_; i++) swarm_odom[i] = Eigen::Vector3d::Zero();

  drone_0_odom_sub_ = nh.subscribe("/drone_0_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_0_odomeCallback, this);
  drone_1_odom_sub_ = nh.subscribe("/drone_1_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_1_odomeCallback, this);
  drone_2_odom_sub_ = nh.subscribe("/drone_2_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_2_odomeCallback, this);
  drone_3_odom_sub_ = nh.subscribe("/drone_3_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_3_odomeCallback, this);
  drone_4_odom_sub_ = nh.subscribe("/drone_4_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_4_odomeCallback, this);
  drone_5_odom_sub_ = nh.subscribe("/drone_5_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_5_odomeCallback, this);
  drone_6_odom_sub_ = nh.subscribe("/drone_6_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_6_odomeCallback, this);
  drone_7_odom_sub_ = nh.subscribe("/drone_7_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_7_odomeCallback, this);
  drone_8_odom_sub_ = nh.subscribe("/drone_8_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_8_odomeCallback, this);
  drone_9_odom_sub_ = nh.subscribe("/drone_9_visual_slam/odom", 1,
                                   &PlanningVisualization::drone_9_odomeCallback, this);
  drone_10_odom_sub_ = nh.subscribe("/drone_10_visual_slam/odom", 1,
                                    &PlanningVisualization::drone_10_odomeCallback, this);
  drone_11_odom_sub_ = nh.subscribe("/drone_11_visual_slam/odom", 1,
                                    &PlanningVisualization::drone_11_odomeCallback, this);
...
{
  if (formation_size_ <= 0) return;

  swarm_odom[0] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_1_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 1) return;

  swarm_odom[1] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_2_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 2) return;

  swarm_odom[2] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_3_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 3) return;

  swarm_odom[3] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_4_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 4) return;

  swarm_odom[4] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_5_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 5) return;

  swarm_odom[5] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}
...
void PlanningVisualization::drone_6_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  ...
  swarm_odom[6] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_7_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 7) return;

  swarm_odom[7] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_8_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 8) return;

  swarm_odom[8] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_9_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 9) return;

  swarm_odom[9] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_10_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 10) return;

  swarm_odom[10] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}

void PlanningVisualization::drone_11_odomeCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (formation_size_ <= 11) return;

  swarm_odom[11] << msg->pose.pose.position.x, msg->pose.pose.position.y,
      msg->pose.pose.position.z;
}
...
void PlanningVisualization::displayGoalPoint(Eigen::Vector3d goal_point,
                                             Eigen::Vector4d color, const double scale,
                                             int id)
{
...
void PlanningVisualization::displayGlobalPathList(vector<Eigen::Vector3d> init_pts,
...
void PlanningVisualization::displayInitPathListDebug(vector<Eigen::Vector3d> init_pts,
                                                     const double scale, int id)
{
    ...
    int id /* = Eigen::Vector4d(0.5,0.5,0,1)*/)
{
  ...
  Eigen::Vector4d color = Eigen::Vector4d(
      0.5 + ((double)rand() / RAND_MAX / 2), 0.5 + ((double)rand() / RAND_MAX / 2), 0,
...
void PlanningVisualization::displayIntermediatePt(std::string type, Eigen::MatrixXd &pts,
...
void PlanningVisualization::displayIntermediateGrad(std::string type,
                                                    Eigen::MatrixXd &pts,
